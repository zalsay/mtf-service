...
const proHeaderGradientStyle = {
  backgroundImage:
    "linear-gradient(90deg, rgba(255,241,184,0.75) 0%, rgba(252,211,77,0.75) 34%, rgba(245,158,11,0.75) 68%, rgba(249,115,22,0.75) 100%)",
};

const liteMetricCardClass =
  "flex flex-col rounded-lg border border-white/10 overflow-hidden h-fit w-full md:w-[100px] md:shrink-0";
const proMetricCardClass =
  "flex flex-col rounded-lg border border-amber-300/35 overflow-hidden h-fit w-full md:w-[100px] md:shrink-0";
const metricsRowClass =
  "grid grid-cols-3 gap-1.5 md:flex md:gap-2 md:overflow-x-auto no-scrollbar";
...
export const StockPredictionSummary: React.FC<StockPredictionSummaryProps> = ({
  ...
  const isProPredPositive =
    (stock.prediction?.proPredictedChangePercent || 0) >= 0;
  const formatContextLen = (value?: number) =>
    value ? (value < 1024 ? value : Math.round(value / 1024) + "K") : "?";

  const confidenceColor =
    (stock.prediction?.confidence ?? 0 > 85)
      ? "text-primary"
      : (stock.prediction?.confidence ?? 0) > 70
        ? "text-yellow-400"
        : "text-red-400";
  const proConfidenceColor =
    (stock.prediction?.proConfidence ?? 0 > 85)
      ? "text-amber-200"
      : (stock.prediction?.proConfidence ?? 0) > 70
        ? "text-yellow-200"
        : "text-orange-300";
  ...
  const watchlistCount =
    typeof stock.watchlistCount === "number" ? stock.watchlistCount : undefined;
  const watchlistCountLabel = t("prediction.watchlistCount");
  const formattedWatchlistCount =
  ...
  const formatContextOption = (value: number) =>
    value < 1024
      ? `${Number((value / 1024).toFixed(1))}k`
      : `${Math.round(value / 1024)}k`;
  const parameterControls =
    hasHorizonOptions || hasContextOptions ? (
            ...
            {horizonOptions!.map((option) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    option.available && onHorizonChange?.(option.value)
                  }
                  ...
                  title={
                    option.available
                      ? `${option.value}d`
                      : language === "zh"
                        ? `暂无 ${option.value}d 数据`
                        : `No ${option.value}d data`
                  }
                >
                  {option.value}d
                </button>
            ...
            {contextOptions!.map((option) => {
              ...
              return (
                <button
                  ...
                  disabled={!option.available}
                  onClick={() =>
                    option.available && onContextChange?.(option.value)
                  }
                  className={`h-8 w-11 rounded-md text-[11px] font-bold transition-colors ${
                    isActive
  ...
  return (
    <div
      className={`flex flex-wrap md:flex-nowrap justify-between items-start gap-x-2 gap-y-3 ${className}`.trim()}
    >
      <div className={`min-w-0 ${nameColumnClassName}`.trim()}>
        {!hideTitle && (
          <>
            <p className="text-white text-lg font-bold leading-normal md:truncate">
              {stock.companyName}
            </p>
        ...
        {((showChartModeToggle && hasChartData) || hasMetrics) && (
          <div className={`${hideTitle ? "" : "mt-3"} flex items-center gap-2`}>
            {showChartModeToggle && hasChartData && (
              <button
                ...
                onClick={() =>
                  onChartModeChange(chartMode === "change" ? "price" : "change")
                }
            ...
            {hasMetrics && (
              <button
                ...
                aria-label={
                  isMetricsExpanded
                    ? t("prediction.hideMetrics")
                    : t("prediction.showMetrics")
                }
                title={
                  isMetricsExpanded
                    ? t("prediction.hideMetrics")
              ...
              >
                <span
                  className={`material-symbols-outlined text-[18px] transition-transform duration-300 ${isMetricsExpanded ? "rotate-180" : ""}`}
                >
                  expand_more
                </span>
              </button>
      ...
      {hasMetrics && (
        <div
          className={`order-3 w-full overflow-hidden transition-all duration-300 ease-out md:order-none md:min-w-0 md:flex-1 ${isMetricsExpanded ? "max-h-[640px] opacity-100" : "max-h-0 opacity-0 md:max-h-none md:opacity-100"}`}
        >
              ...
              {hasLiteMetrics && (
                <div className={metricsRowClass}>
                  <div className={liteMetricCardClass}>
                    <div
                      className="px-2 py-1 flex items-center gap-1.5 justify-center"
                      style={liteHeaderStyle}
                    >
                      ...
                      <span className="text-xs font-bold text-white/90 leading-tight">
                        {stock.prediction.contextLen
                          ? stock.prediction.contextLen < 1024
                            ? stock.prediction.contextLen
                            : Math.round(stock.prediction.contextLen / 1024) +
                              "K"
                          : "?"}
                      </span>
                    </div>
                  ...
                  <div className={liteMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span className="text-xs font-bold text-white/90 leading-tight">
                        {stock.prediction.maxDeviationPercent?.toFixed(2) ??
                          "0.00"}
                        %
                      </span>
                    </div>
                  ...
                  <div className={liteMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span
                        className={`text-xs font-bold leading-tight ${confidenceColor}`}
                      >
                  ...
                  {stock.predictedChangePercent !== undefined &&
                    stock.predictedChangePercent !== 0 && (
                      <div className={liteMetricCardClass}>
                        <div
                          className="px-2 py-1 flex items-center gap-1.5 justify-center"
                          style={liteHeaderStyle}
                        >
                          ...
                          <span className="text-xs font-medium text-white/80 leading-none">
                            {t("prediction.predChg")}
                          </span>
                        </div>
                        <div className="bg-white/5 px-2 py-1 flex justify-center">
                          <span className="text-xs font-bold leading-tight text-primary">
                            {isPredPositive ? "+" : "-"}
                            {Math.abs(stock.predictedChangePercent).toFixed(2)}%
                          </span>
                        </div>
                      </div>
                    )}
                ...
                <div className={metricsRowClass}>
                  ...
                  <div className={proMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span className="text-xs font-bold text-amber-100 leading-tight">
                        {formatContextLen(
                          stock.prediction.proContextLen ||
                            stock.prediction.contextLen,
                        )}
                      </span>
                    </div>
                  ...
                  <div className={proMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span className="text-xs font-bold text-amber-100 leading-tight">
                        {stock.prediction.proHorizonLen ||
                          stock.prediction.horizonLen ||
                          "?"}{" "}
                        {t("prediction.days")}
                      </span>
                  ...
                  <div className={proMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span className="text-xs font-bold text-amber-100 leading-tight">
                        {stock.prediction.proMaxDeviationPercent !== undefined
                          ? `${stock.prediction.proMaxDeviationPercent.toFixed(2)}%`
                          : "—"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={proMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span
                        className={`text-xs font-bold leading-tight ${stock.prediction.proConfidence !== undefined ? proConfidenceColor : "text-amber-100"}`}
                      >
                        {stock.prediction.proConfidence !== undefined
                          ? stock.prediction.proConfidence.toFixed(4)
                          : "—"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={proMetricCardClass}>
                    ...
                    <div className="bg-white/5 px-2 py-1 flex justify-center">
                      <span
                        className={`text-xs font-bold leading-tight ${stock.prediction.proPredictedChangePercent !== undefined ? (isProPredPositive ? "text-amber-200" : "text-orange-300") : "text-amber-100"}`}
                      >
                        {stock.prediction.proPredictedChangePercent !==
                        undefined
        ...
        {(watchlistCount !== undefined || onAddToWatchlist) && (
          ...
          >
            <span
              ...
              style={{
                fontVariationSettings: `'FILL' ${isWatchlisted ? 1 : 0}`,
              }}
            >
              favorite
            </span>
            ...
            <span
              className={`text-sm font-black leading-none ${isWatchlisted ? "text-primary" : "text-white/88"}`}
            >
...
const StockPredictionCard: React.FC<StockPredictionCardProps> = ({
  ...
  const { t } = useLanguage();
  const [internalChartMode, setInternalChartMode] =
  ...
  return (
    <div
      className={`flex flex-col gap-4 rounded-xl ${borderless ? "" : "border border-white/10"} bg-card-dark p-6 ${className}`.trim()}
    >
      {!hideSummary && (
      ...
      <div
        className={`flex flex-1 flex-col gap-4 py-4 ${chartHeightClassName}`.trim()}
      >
