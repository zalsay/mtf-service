...
import { PredictionChartData, PublicPredictionItem, StockData } from '../../types';
...
import StockPredictionCard, { StockPredictionSummary } from '../dashboard/StockPredictionCard';
...
const normalizePredictionSymbol = (symbol: string): string => {
    const trimmed = String(symbol || '').trim().toLowerCase();
...
const getTrainingStockCode = (item: WatchlistItem | null): string => {
    const symbol = String(item?.stock?.symbol || '').trim().toLowerCase();
    if (!symbol) {
        return '';
    }
...
const selectLatestPredictionItem = (
    ...
    return comparePredictionUpdatedAt(current.best.updated_at, next.best.updated_at) <= 0 ? next : current;
};
...
const getChartModelMeta = (
    ...
    if (hasPro) {
        return { group: 'pro', label: language === 'zh' ? 'Pro 模型' : 'Pro Model' };
    }
    return { group: 'lite', label: language === 'zh' ? 'Lite 模型' : 'Lite Model' };
};
...
const formatChartHorizonLabel = (horizonLen: number, language: string): string => (
    horizonLen
        ? `${horizonLen}${language === 'zh' ? '天' : 'd'}`
        : '?'
);
...
const buildChartPredictionOptions = (
    ...
    return Array.from(grouped.entries())
        ...
        .sort((left, right) => {
            ...
            const modelDiff = getChartModelSortWeight(left.modelGroup) - getChartModelSortWeight(right.modelGroup);
            if (modelDiff !== 0) {
                return modelDiff;
            }
...
const CustomTrainSelect = <T extends string | number>({
    ...
    const [menuPosition, setMenuPosition] = useState({ left: 0, top: 0, width: 0, maxHeight: 240 });
    ...
    const updateMenuPosition = () => {
        ...
        const maxHeight = Math.min(preferredMaxHeight, Math.max(140, availableSpace - gap));
    ...
    return (
        <div ref={containerRef} className="relative space-y-2">
            ...
            >
                ...
                <span className={`material-symbols-outlined text-[18px] text-white/55 transition-transform ${open ? 'rotate-180 text-primary' : ''}`}>
                    expand_more
                </span>
            </button>
...
const Watchlist: React.FC<WatchlistProps> = ({ initialStocks, onAuthError }) => {
    ...
    const [singlePredictionItem, setSinglePredictionItem] = useState<WatchlistItem | null>(null);
    const [singlePredictionMode, setSinglePredictionMode] = useState<'standalone' | 'next_chunk'>('standalone');
    const [nextChunkBestItem, setNextChunkBestItem] = useState<PublicPredictionItem | null>(null);
    const [selectedStock, setSelectedStock] = useState<StockData | null>(null);
    const [selectedWatchlistItem, setSelectedWatchlistItem] = useState<WatchlistItem | null>(null);
    const [chartOpen, setChartOpen] = useState(false);
    const [chartModalMode, setChartModalMode] = useState<PredictionChartMode>('price');
    ...
    const [selectedResolvedPrediction, setSelectedResolvedPrediction] = useState<ResolvedPublicPrediction | null>(null);
    const [chartPredictionOptions, setChartPredictionOptions] = useState<ChartPredictionOption[]>([]);
    const [activeChartPredictionOptionId, setActiveChartPredictionOptionId] = useState<string | null>(null);
    ...
    const trainContextOptions: TrainSelectOption<number>[] = trainPolicy.contextLens.map(option => ({
        value: option,
        label: String(option),
    }));
    const trainHorizonOptions: TrainSelectOption<number>[] = trainPolicy.horizonLens.map(option => ({
        value: option,
    ...
    const loadWatchlist = async () => {
        try {
            ...
            const validWatchlistItems = (response.watchlist || []).filter((item: WatchlistItem | null | undefined): item is WatchlistItem => !!item && !!item.stock?.symbol);
            setWatchlistItems(validWatchlistItems);
            const symbols = validWatchlistItems.map((it: WatchlistItem) => getItemSymbol(it)).filter(Boolean) as string[];
            if (symbols.length > 0) {
                try {
    ...
    const handleAddStock = async (symbol: string, type: 1 | 2) => {
        try {
            ...
            const exists = watchlistItems.some(it => getItemSymbol(it).toLowerCase() === symLower);
            if (exists) {
    ...
    const confirmRemove = (id: number) => {
        const sym = getItemSymbol(watchlistItems.find(it => it?.id === id)).toLowerCase();
    ...
    const applyChartPredictionOption = (
        ...
        const loadedPredictionType = preferredPredictionType || getLoadedPredictionType(option.resolved);
        const loadedItem = getPredictionItemForType(option.resolved, loadedPredictionType) || option.resolved.primary;
        ...
        const displayCompanyName = itemCompanyName
            || (mappedCompanyName && mappedCompanyName !== mapped.symbol ? mappedCompanyName : symbol);
        setSelectedStock({
            ...mapped,
    ...
    const handleChartParameterChange = (
        ...
        const option = pickClosestChartOption(chartPredictionOptions, activeChartPredictionOption, patch);
    ...
    const loadChartPrediction = async (
        ...
        try {
            ...
            if (res && res.items) {
                const options = buildChartPredictionOptions(res.items, normalizedSymbol, language);
                setChartPredictionOptions(options);
                const found = options.find(option => matchesTrainResultPreference(option.resolved, preference)) || options[0];
                if (found) {
                    if (requestSeq !== chartRequestSeqRef.current) {
                        return;
                    }
    ...
    const resolveTrainResultPreference = (
        ...
        ...fallback,
        covariateSignature: status?.covariate_signature || accepted.covariate_signature || fallback.covariateSignature,
    });
    ...
    const pollBestTrainingJob = async (
        ...
        for (let attempt = 0; attempt < BEST_TRAIN_MAX_POLL_ATTEMPTS; attempt += 1) {
            if (requestSeq !== chartRequestSeqRef.current) {
                return;
            }
            ...
            if (nextStatus.status === 'succeeded') {
                const resolvedPreference = resolveTrainResultPreference(accepted, nextStatus, fallbackPreference);
    ...
    const getCurrentBestPredictionItem = (): PublicPredictionItem | null => {
        const item = getPredictionItemForType(selectedResolvedPrediction, trainPredictionType);
        if (!item) {
            return null;
        }
    ...
    const openNextChunkPredictionModal = () => {
		...
		if (!bestItem) {
			setChartError(language === 'zh' ? '暂无可用于回测的预测结果，请先执行 MTF 预测推理。' : 'No prediction result is available for backtesting. Run MTF prediction first.');
			return;
		}

        const stockCode = getTrainingStockCode(selectedWatchlistItem);
        if (!stockCode) {
            setChartError(language === 'zh' ? '股票代码无效，暂时无法预测' : 'Invalid stock code');
            return;
        }
    ...
    const handleNextChunkPredictionComplete = (
		...
		if (!bestItem) {
			throw new Error(language === 'zh' ? '暂无可用于回测的预测结果，请先执行 MTF 预测推理。' : 'No prediction result is available for backtesting. Run MTF prediction first.');
		}
        ...
        if (!nextChartData) {
            throw new Error(language === 'zh' ? 'MTF 预测完成，但无法生成下一段图表' : 'MTF prediction finished but chart data is unavailable');
        }
    ...
    const handleTrainBest = async () => {
        ...
        try {
            ...
            const estimatedSeconds = normalizeEstimatedSeconds(accepted.estimated_inference_time_sec);
            if (estimatedSeconds) {
                setTrainingProgress({
                    estimatedSeconds,
                    startedAt: Date.now(),
            ...
            trainResultPreferenceRef.current = resolveTrainResultPreference(accepted, null, resultPreference);
    ...
    const chartDisplayStock = selectedStock && selectedStock.prediction
        ? {
            ...selectedStock,
            prediction: {
    ...
    const shouldShowTrainPanel = !!selectedStock && (showTrainPanel || !selectedStock.prediction);
    ...
    const trainingProgressPercent = trainingProgress
        ? Math.min(98, Math.max(0, (trainingElapsedSeconds / trainingProgress.estimatedSeconds) * 100))
        : 0;
            ...
            >
                ...
                >
                    <div className="flex items-start justify-between gap-3">
                        ...
                        >
                            {isTrainingBest
                                ? (language === 'zh' ? `训练中 ${selectedStock.symbol}` : `Training ${selectedStock.symbol}`)
                                : (language === 'zh' ? `训练 ${selectedStock.symbol}` : `Train ${selectedStock.symbol}`)}
                        </button>
                    </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        <CustomTrainSelect
                            ...
                            disabled={isTrainingBest || trainPredictionTypeOptions.length <= 1}
                            onChange={value => {
                        ...
                        <div className="flex flex-col gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3">
                            <div className="flex items-center justify-between gap-3 text-xs font-medium text-white/70">
                                <span className="truncate">
                                    {trainingProgress
                                        ? (language === 'zh' ? '预计剩余' : 'Estimated remaining')
                                        : (language === 'zh' ? '已提交训练任务' : 'Training request submitted')}
                                </span>
                                {trainingProgress && (
                                ...
                                {trainingProgress ? (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            ...(isProTraining ? { background: PRO_PROGRESS_GRADIENT } : {}),
                                        }}
                                    />
                                ) : (
                                    <div
                                        className={`h-full w-0 rounded-full ${isProTraining ? '' : 'bg-primary/70'}`}
                                        style={isProTraining ? { background: PRO_PROGRESS_GRADIENT } : undefined}
                                    />
                                )}
                            </div>
                        </div>
            ...
            <div className="flex flex-nowrap items-center justify-end gap-2">
                {chartDisplayStock?.prediction?.chartData && (
                    <button
                        ...
                        onClick={() => setChartModalMode(prev => prev === 'change' ? 'price' : 'change')}
                ...
                <button
                    ...
                    disabled={!selectedStock?.prediction || isPredictingNextChunk || isTrainingBest}
                ...
                >
                    {isPredictingNextChunk
                        ? (language === 'zh' ? '预测中' : 'Running')
                        : (language === 'zh' ? 'MTF 预测' : 'Predict')}
                </button>
                <button
                    type="button"
    ...
    const renderChartParameterSelector = (className = '', actionSlot?: React.ReactNode) => {
        ...
        <div className="flex flex-col w-full">
            <div className="flex flex-wrap justify-between gap-3 p-4 mb-4">
                ...
                >
                    ...
                    <span>{t('uzi.refresh', language === 'zh' ? '刷新' : 'Refresh')}</span>
                </button>
            </div>
                ...
                <div className="flex items-center gap-2 lg:justify-between">
                    ...
                    <div className="ml-auto flex min-w-0 flex-1 items-center justify-end gap-2 sm:flex-none">
                        ...
                        >
                            ...
                            <span className="hidden truncate sm:inline">{t('watchlist.addStock')}</span>
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex h-[78vh] max-h-[92vh] w-full flex-col overflow-hidden rounded-t-[28px] border border-white/10 bg-card-dark md:h-auto md:max-w-6xl md:rounded-xl xl:max-w-[1320px]">
                         <div className="border-b border-white/10 p-4 md:p-6">
                             ...
                             <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                ...
                                {chartPredictionOptions.length <= 1 && renderChartHeaderActions('shrink-0 pr-1 md:pr-2')}
                             </div>
                         </div>
                         <div className="relative flex min-h-0 flex-1 flex-col overflow-y-auto bg-card-dark p-4 md:min-h-[300px] md:p-6">
                             {isChartLoading ? (
                                 ...
                                 <>
                                     ...
                                     {chartError && selectedStock.prediction && !shouldShowTrainPanel && (
                        ...
                        <table className="min-w-[920px] w-full">
                            <thead className="border-b border-b-[#2D2D2D]">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-center text-white/60 text-sm font-medium leading-normal">{t('watchlist.singlePredict')}</th>
                                    <th className="px-4 py-3 text-center text-white/60 text-sm font-medium leading-normal">{t('watchlist.showChart')}</th>
