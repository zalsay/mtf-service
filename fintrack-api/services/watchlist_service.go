...
const timesfmBestStaleRefreshAfter = 180 * 24 * time.Hour
...
func normalizeTimesfmSymbolReadKey(symbol string) string {
...
func timesfmCanonicalSymbolExpr(column string) string {
	return fmt.Sprintf(
...
type timesfmMembershipTrainPolicy struct {
...
func getTimesfmMembershipTrainPolicy(level int) timesfmMembershipTrainPolicy {
	switch level {
	case 3:
		return timesfmMembershipTrainPolicy{
			AllowedPredictionTypes: newStringSet("non_cov", "cov"),
	...
	default:
		return timesfmMembershipTrainPolicy{
			AllowedPredictionTypes: newStringSet("non_cov"),
...
func NormalizeTimesfmBestTrainRequest(req *models.TimesfmBestTrainRequest, membershipLevel int, userID int, isAdmin bool) (*models.TimesfmPredictRequest, error) {
	if req == nil {
		return nil, fmt.Errorf("request is required")
	}

	policy := getTimesfmMembershipTrainPolicy(membershipLevel)
	...
	normalized := &models.TimesfmPredictRequest{
...
func NormalizeTimesfmPredictOnceRequest(req *models.TimesfmPredictRequest, membershipLevel int, userID int, isAdmin bool) (*models.TimesfmPredictRequest, error) {
	if req == nil {
		return nil, fmt.Errorf("request is required")
	}
...
func normalizeTimesfmPredictStockType(value interface{}) int {
...
func (s *WatchlistService) triggerStaleTimesfmBestRefresh(item models.TimesfmBestPrediction) error {
	if s == nil || s.config == nil || strings.TrimSpace(s.config.PythonService.BaseURL) == "" {
		return nil
	}
	if item.UpdatedAt.IsZero() || time.Since(item.UpdatedAt) <= timesfmBestStaleRefreshAfter {
		return nil
	}
	...
	request := &models.TimesfmPredictRequest{
	...
	if strings.TrimSpace(item.TimesfmVersion) != "" {
		timesfmVersion := strings.TrimSpace(item.TimesfmVersion)
		request.TimesfmVersion = &timesfmVersion
	}
	...
	if request.PredictionType == "cov" && request.CovariateConfig == nil {
		request.CovariateConfig = buildMarketCovariateConfig()
		preset := "market_cov_v1"
		request.CovariatePreset = &preset
	}

	_, _, err := s.TriggerTimesfmPredict(request)
	return err
}

func (s *WatchlistService) TriggerTimesfmPredictOnce(req *models.TimesfmPredictRequest) (int, map[string]interface{}, error) {
	payload := buildTimesfmPredictPayload(req)
	jsonData, err := json.Marshal(payload)
	if err != nil {
...
func buildTimesfmPredictPayload(req *models.TimesfmPredictRequest) map[string]interface{} {
	payload := map[string]interface{}{
		"stock_code": req.StockCode,
	}
	if req.StockType != nil {
...
func (s *WatchlistService) GetTimesfmPredictOnceCached(req *models.TimesfmPredictRequest) (int, map[string]interface{}, error) {
	if s == nil || s.config == nil || strings.TrimSpace(s.config.PythonService.BaseURL) == "" {
		return 0, nil, fmt.Errorf("python service is not configured")
	}
	if req == nil {
	...
	requestURL := fmt.Sprintf("%s/predict_once_cached", s.config.PythonService.BaseURL)
	client := newPythonServiceHTTPClient(s.config.PythonService.Timeout)
	resp, err := client.Post(requestURL, "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
		return 0, nil, fmt.Errorf("call python predict once cached: %v", err)
	}
	defer resp.Body.Close()

	body, err := readPythonJSONResponse(resp, requestURL, "decode python predict once cached response")
	if err != nil {
		return resp.StatusCode, nil, err
	}
	return resp.StatusCode, body, nil
...
func (s *WatchlistService) GetTimesfmJobStatus(jobID string) (int, map[string]interface{}, error) {
...
func (s *WatchlistService) GetTimesfmBestByUniqueKey(uniqueKey string) (*models.TimesfmBestPrediction, error) {
	row := s.db.Conn.QueryRow(`
        SELECT 
            id, unique_key, symbol, timesfm_version, best_prediction_item, best_metrics::text,
...
func (s *WatchlistService) GetTimesfmBestUniqueKeysByConfig(symbol string, horizonLen int, contextLen int, timesfmVersion string) (*models.TimesfmBestUniqueKeysByConfig, error) {
	normalizedSymbol := normalizeTimesfmSymbolReadKey(symbol)
	canonicalSymbolExpr := timesfmCanonicalSymbolExpr("symbol")
    ...
    `, canonicalSymbolExpr, canonicalSymbolExpr), normalizedSymbol, horizonLen, contextLen, strings.TrimSpace(timesfmVersion))
	if err != nil {
		return nil, fmt.Errorf("failed to query timesfm_best_predictions by config: %v", err)
	}
	defer rows.Close()

	result := &models.TimesfmBestUniqueKeysByConfig{
		Symbol:          normalizedSymbol,
		TimesfmVersion:  strings.TrimSpace(timesfmVersion),
		HorizonLen:      horizonLen,
		ContextLen:      contextLen,
		NonCovUniqueKey: "",
		CovUniqueKey:    "",
	...
	for rows.Next() {
		...
		if err := rows.Scan(&predictionType, &uniqueKey); err != nil {
			return nil, fmt.Errorf("failed to scan timesfm_best_predictions by config: %v", err)
		}
		switch normalizedPredictionType(predictionType) {
		case "cov":
			result.CovUniqueKey = uniqueKey
			found++
		default:
			result.NonCovUniqueKey = uniqueKey
			found++
		}
	}
	if found == 0 {
...
func (s *WatchlistService) listScopedTimesfmBest(horizonLen int, symbol string, userID *int, includePrivate bool) ([]models.TimesfmBestPrediction, error) {
	baseCanonicalSymbolExpr := timesfmCanonicalSymbolExpr("symbol")
	rowCanonicalSymbolExpr := timesfmCanonicalSymbolExpr("t.symbol")
	query := fmt.Sprintf(`
	...
	if includePrivate {
		...
		conditions = append(conditions, fmt.Sprintf(`(
			is_public = 1 OR EXISTS (
				SELECT 1
				FROM timesfm_best_validation_chunks vc
				WHERE vc.unique_key = timesfm_best_predictions.unique_key
				  AND vc.user_id = $%d
			)
		)`, len(args)))
	...
	if normalizedSymbol := normalizeTimesfmSymbolReadKey(symbol); normalizedSymbol != "" {
	...
	query += fmt.Sprintf(`
        ...
        watchlist_counts AS (
            SELECT
                %s AS canonical_symbol,
                COUNT(DISTINCT user_id)::int AS watchlist_count
            FROM user_watchlist
            GROUP BY %s
        ),
        ranked AS (
            SELECT
                ...
                ROW_NUMBER() OVER (
                    PARTITION BY %s, t.horizon_len, t.context_len, t.timesfm_version, COALESCE(NULLIF(TRIM(t.prediction_type), ''), 'non_cov')
                    ORDER BY CASE WHEN %s = lower(trim(t.symbol)) THEN 0 ELSE 1 END,
                             t.updated_at DESC,
                             t.id DESC
                ) AS rn
            FROM timesfm_best_predictions t
            INNER JOIN public_groups g
                ON g.canonical_symbol = %s
               AND g.horizon_len = t.horizon_len
               AND g.context_len = t.context_len
               AND g.timesfm_version = t.timesfm_version
        ...
        SELECT
            ...
            val_start_date, val_end_date,
            context_len, horizon_len,
            created_at, updated_at, short_name, watchlist_count
        FROM ranked
        WHERE rn = 1
        ORDER BY updated_at DESC
    `, timesfmCanonicalSymbolExpr("symbol"), timesfmCanonicalSymbolExpr("symbol"), rowCanonicalSymbolExpr, rowCanonicalSymbolExpr, rowCanonicalSymbolExpr, rowCanonicalSymbolExpr, rowCanonicalSymbolExpr)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.ValStartDate, &item.ValEndDate,
			&item.ContextLen, &item.HorizonLen,
			&item.CreatedAt, &item.UpdatedAt, &item.ShortName, &item.WatchlistCount,
		); err != nil {
...
func (s *WatchlistService) ListPublicTimesfmBest(horizonLen int, symbol string, includePrivate bool) ([]models.TimesfmBestPrediction, error) {
	return s.listScopedTimesfmBest(horizonLen, symbol, nil, includePrivate)
}

func (s *WatchlistService) ListAccessibleTimesfmBest(userID int, horizonLen int, symbol string, includePrivate bool) ([]models.TimesfmBestPrediction, error) {
	uid := userID
	items, err := s.listScopedTimesfmBest(horizonLen, symbol, &uid, includePrivate)
	if err != nil {
		return nil, err
	}
	for _, item := range items {
		if err := s.triggerStaleTimesfmBestRefresh(item); err != nil {
			log.Printf("stale timesfm best refresh enqueue failed: symbol=%s unique_key=%s err=%v", item.Symbol, item.UniqueKey, err)
		}
	}
	return items, nil
}
...
func (s *WatchlistService) ListValidationChunksByUniqueKeys(uniqueKeys []string) (map[string][]models.SaveTimesfmValChunkRequest, error) {
	keys := uniqueNonEmptyStrings(uniqueKeys)
	chunksByKey := make(map[string][]models.SaveTimesfmValChunkRequest, len(keys))
	for _, key := range keys {
		chunksByKey[key] = []models.SaveTimesfmValChunkRequest{}
	}
	if len(keys) == 0 {
	...
	rows, err := s.db.Conn.Query(`
        SELECT 
            ...
            dates, COALESCE(prediction_type, 'non_cov'),
            covariate_config, covariate_signature, covariate_analysis
	...
	for rows.Next() {
		req, err := scanTimesfmValidationChunk(rows)
		if err != nil {
			return nil, err
		}
...
func scanTimesfmValidationChunk(rows *sql.Rows) (models.SaveTimesfmValChunkRequest, error) {
...
func (s *WatchlistService) GetLatestValidationChunkByUniqueKey(uniqueKey string) (map[string]interface{}, error) {
	row := s.db.Conn.QueryRow(`
        SELECT
            ...
            vc.start_date,
            vc.end_date,
            COALESCE(vc.prediction_type, 'non_cov'),
